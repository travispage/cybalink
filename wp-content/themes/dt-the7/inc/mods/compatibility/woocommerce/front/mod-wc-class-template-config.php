...
if ( ! class_exists( 'DT_WC_Template_Config', false ) ) :
	...
	class DT_WC_Template_Config {
		...
		public function setup() {
			...
			$config->set( 'post.preview.description.style', of_get_option( 'woocommerce_display_product_info', 'wc_btn_on_hoover' ), 'wc_btn_on_hoover' );
            ...
            $config->set( 'layout', $wc_layout, 'masonry' );
            if(of_get_option( 'wc_view_mode' ) == 'view_mode' ){
	            $wc_layout_switch = 'masonry';
	            switch ( the7_get_view_mode()) {
	                case 'masonry_grid':
	                    if ( 'grid' == of_get_option( 'woocommerce_shop_template_isotope' ) ) {
	                        $wc_layout_switch = 'grid';
	                    }else{
	                    	$wc_layout_switch = 'masonry';
	                    }
	                    break;
	                case 'list':
	                    $wc_layout_switch = 'list';
	                    break;
	            }

	            $config->set( 'layout', $wc_layout_switch, 'masonry' );
	        }
