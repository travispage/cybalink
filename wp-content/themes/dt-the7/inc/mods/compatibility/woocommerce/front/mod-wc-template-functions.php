...
function dt_woocommerce_get_alt_product_thumbnail() {
    if ( of_get_option( 'woocommerce_hover_image' ) ) {
	    ...
	    $class = 'show-on-hover back-image preload-me';
	    $size = 'shop_catalog';
...
if(!function_exists('the7_get_view_mode')) {
	function the7_get_view_mode() {
		...
		//$new_session = new WC_Session_Handler(); // WC()->session
		$mode = of_get_option('wc_view_mode');
		$current_mode = 'masonry_grid';

		// if(of_get_option('woocommerce_shop_template_layout_default') == 'list_default'){
		// 	$current_mode = 'list';
		// }else{
		// 	if(of_get_option('woocommerce_shop_template_isotope') == 'masonry'){
		// 		$current_mode = 'masonry';
		// 	}else{
		// 		$current_mode = 'grid';
		// 	}
		// }
		...
		// elseif ( $new_session->__isset( 'wc_view_mode' ) &&  (is_shop() || is_product_category() || is_product_tag())) :
		// 	$current_mode = ( $new_session->__get( 'wc_view_mode' ) );
		// var_dump($current_mode);
		// elseif ( $mode == 'masonry_grid_default' ) :
		// 	if(of_get_option('woocommerce_shop_template_isotope') == 'masonry'){
		// 		$current_mode = 'masonry';
		// 	}else{
		// 		$current_mode = 'grid';
...
// if(!function_exists('the7_view_mode_action')) {
// 	add_action( 'init', 'the7_view_mode_action', 100 );
// 	function the7_view_mode_action() {
// 		if ( isset( $_REQUEST['wc_view_mode'] ) ) :
// 			if( ! class_exists('WC_Session_Handler')) return;
// 			$new_session = new WC_Session_Handler(); // WC()->session
// 			$new_session->set( 'wc_view_mode', ( $_REQUEST['wc_view_mode'] ) );
// 		endif;
// 	}
// }
